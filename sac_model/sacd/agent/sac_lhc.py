...
class SAC_LHC(BaseAgent):
    ...
    def __init__(self, env, test_env, log_dir, num_steps=100000, batch_size=64,
        ...
        if not self.continuous:
            ...
            self.Q1 = QNetwork(self.obs_dim[-1],num_actions=env.action_space.shape[0],cnn=cnn,continuous=True).to(device=self.device)
            self.Q2 = QNetwork(self.obs_dim[-1],num_actions=env.action_space.shape[0],cnn=cnn,continuous=True).to(device=self.device)
            self.value = ValueNetwork(self.obs_dim[-1],cnn=cnn).to(device=self.device)
            self.value_target = ValueNetwork(self.obs_dim[-1],cnn=cnn).to(device=self.device)
    ...
    def train_body(self,batch):
        ...
        policy_loss = torch.mean(self.alpha * logp - current_min_q)
